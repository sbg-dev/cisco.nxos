    ...
    def _compare_lists(self, want, have):
        ...
        if self.state in ("replaced", "overridden"):
            ...
            if not want_set and have_set:
                ...
                self.commands.append("no switchport trunk allowed vlan")
            elif want_set != have_set:
                # Exact-state operations must replace the complete list. This
                # also handles a new trunk correctly: NX-OS implicitly allows
                # all VLANs, so an incremental "add" would have no effect.
                self.commands.extend(
                    generate_switchport_trunk(
                        "allowed",
                        False,
